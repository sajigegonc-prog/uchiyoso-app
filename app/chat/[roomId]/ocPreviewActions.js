...
export async function getOcDetailForRoom(ocId, roomId) {
  ...
  const { data: memberships } = await supabase.from('chat_room_members').select('id').eq('room_id', roomId).eq('user_id', user.id).limit(1)
  if (!memberships || memberships.length === 0) return { error: '権限がありません' }
