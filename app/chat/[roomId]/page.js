...
export default async function ChatRoomPage({ params }) {
  ...
  const { data: messages } = await supabase
    .from('messages')
    .select('id, content, created_at, sender_oc_id, ocs(name)')
    .eq('room_id', roomId)
    ...
    <div style={{ fontFamily: "'BIZ UDPGothic', sans-serif", background: '#eee1cb', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: '18px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {messages && messages.map((msg) => {
          const mine = myOcs.some((oc) => oc.id === msg.sender_oc_id)
          return (
            <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: mine ? 'flex-end' : 'flex-start' }}>
              <div style={{ fontSize: 10.5, color: '#8b7355', marginBottom: 2 }}>{msg.ocs?.name}</div>
              <div style={{
      ...
      {myOcs.length > 0 ? (
        <MessageForm action={sendMessage} roomId={room.id} myOcs={myOcs} />
      ) : (
