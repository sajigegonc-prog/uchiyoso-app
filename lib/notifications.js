export async function getNotifications(supabase, userId) {
  ...
  const { data: memberships } = await supabase
    ...
    .select('room_id, oc_id, last_read_at, ooc_last_read_at')
    .eq('user_id', userId)
    .is('left_at', null)

  let hasUnreadMessage = false
  let hasUnreadOoc = false
  ...
  if (memberships && memberships.length > 0) {
    ...
    const { data: latestOocMsgs } = await supabase
      .from('room_ooc_messages')
      .select('room_id, user_id, created_at')
      .in('room_id', roomIds)
      .order('created_at', { ascending: false })
    const seenOocRoom = new Set()
    for (const msg of latestOocMsgs || []) {
      if (seenOocRoom.has(msg.room_id)) continue
      seenOocRoom.add(msg.room_id)
      if (selfOnlyRooms.has(msg.room_id)) continue
      if (msg.user_id === userId) continue
      const lastRead = oocLastReadByRoom.get(msg.room_id)
      if (!lastRead || new Date(msg.created_at) > new Date(lastRead)) {
        hasUnreadOoc = true
        break
      }
  ...
  const { data: pendingFriendRequests } = await supabase
    ...
    .eq('status', 'pending')
    .limit(1)
  const { data: newlyAcceptedFriendships } = await supabase
    .from('friendships')
    .select('id')
  ...
  const hasFriendNotification =
    ...
    (newlyAcceptedFriendships && newlyAcceptedFriendships.length > 0)

  return {
    chat: hasUnreadMessage || hasUnreadOoc || (pendingInvites && pendingInvites.length > 0),
    owl: hasUnreadLetter,
    matching: false,
    friends: hasFriendNotification,
  }
}
